...
public class RSAHelper {
	...
	public static boolean topVerify(String data,String sign,String key,String instId)
	{
    	...
    	try 
    	{
			 result = RSACore.verify(Coderc.decryptBASE64(data), SecretKeyManager.getSecretKey(instId+key), sign);
		} catch (Exception e) {
	...
	public static String topSign(String data,String key,String instId)
	{
		...
		try 
		{
			signValue = RSACore.sign(Coderc.decryptBASE64(data), SecretKeyManager.getSecretKey(instId+key));
		} catch (Exception e) {
			logger.error("签名异常：", e);
		}
		return signValue;
